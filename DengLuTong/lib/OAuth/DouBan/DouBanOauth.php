<?php
namespace DengLuTong\lib\OAuth\DouBan;
class DouBanOauth{
	...
	public function fetchToken($url = NULL)
	{
		$httpRequest = $this->http($url, 'GET');
		return $this->parse($httpRequest);
	...
	public function getRequestToken()
	{
		...
		return $this->fetchToken($oauthRequest->to_url());
	}	
	...
	public function getAccessToken($args=array())
	{
		if ($args['key'] && $args['secret']) {
			$token = new OAuthToken($args['key'], $args['secret']);
		}
	...
	public function accessResource($method = NULL, $url = NULL)
	{
		...
		$oauthRequest->sign_request($this->_method, $this->_consumer, $this->_token);
		//$headers = $oauthRequest->to_header();  //自带方法会提示签名错误
		if(!$this->_consumer || !$this->_token)return FALSE;
		$headers = $this->build_signature1($this->_consumer->key, $this->_consumer->secret, $this->_token->key, $this->_token->secret, $url, $method, array());
		$headers=(array)$headers;
		if (($method == 'POST')||($method == 'PUT')) {
			$headers['Content-Type'] = 'application/atom+xml; charset=utf-8';
		}
		$httpRequest = $this->http($url, $method,NULL,FALSE,$headers);
		return $httpRequest;
	}
	
  function build_signature1($consumer_key, $consumer_secret, $oauth_token, $oauth_token_secret, $url, $method, $parameters ){ 
    $mt = microtime(); 
    $rand = mt_rand(); 
    $nonce = md5($mt.$rand);	
    $time = time();	
    ...
    $key=$consumer_secret."&".($oauth_token_secret?$oauth_token_secret:"");	
    
    $base_string=$method.'&'.urlencode($url).'&'.urlencode($post_string); 
    $signature=base64_encode(hash_hmac('sha1',$base_string, $key, true)); 
    
    $post = array( 
    'realm'=>urlencode('http://'.$_SERVER['SERVER_NAME']), 
    'oauth_consumer_key' => $consumer_key, 
    'oauth_nonce' => $nonce, 
    ...
    $header_string='Authorization: OAuth '.$header_string; 
	 ...
	 */
	function http($url, $method, $postfields = NULL , $multi = false,$header=array()) {
		...
		if( $multi )
			$header[] = array("Content-Type: multipart/form-data; boundary=" . OAuthUtil::$boundary , "RemoteIP: " . $_SERVER['REMOTE_ADDR'] , "Expect: ");
